...
class OrderDetails extends Component {
    constructor(props) {
        ...
        console.log("products", this.props.route.params.OrderDataa.product_id);
    }
    render() {
        return (
            <View>
                ...
                <View style={{ marginTop: 20, backgroundColor: '#fff', alignSelf: 'center', borderRadius: 15, width: '90%' }}>
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 5 }}>
                        ...
                        <View style={{ justifyContent: 'flex-end' }}><Text style={{ paddingRight: 15, fontSize: 15 }}>PKR {JSON.parse(this.props.route.params.OrderDataa.total_bill)}</Text></View>
                    </View>
                    ...
                    <FlatList
                        data={JSON.parse(this.props.route.params.OrderDataa.product_id)}
                        renderItem={({ item, index }) => {
                            console.log(item);
                            return (

                                <View style={{ flexDirection: 'row', justifyContent: 'space-around', marginTop: 15 }}>
                                    <View style={{ justifyContent: 'flex-start' }}><Text style={{ fontSize: 15, }}>{item.name}</Text></View>
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 15 }}>
                        ...
                        <View style={{ justifyContent: 'flex-end' }}><Text style={{ paddingRight: 15, fontSize: 15, }}>{JSON.parse(this.props.route.params.OrderDataa.total_bill)}</Text></View>
                    </View>
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 15 }}>
